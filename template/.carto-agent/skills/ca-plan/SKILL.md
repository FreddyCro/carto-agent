...
- 建檔時在工作檔開頭記錄 `Baseline: {git rev-parse HEAD 的結果}`，供 `/ca-close` 界定本次 issue 的 diff 範圍（避免長壽分支上重複 review 累積 diff）
...
> **不可跳過，但可快速通過。** 在投入 PLAN.md 與 worktree 前，先把模糊處問清楚、把方向選定。改方向在這裡只是改文字，到實作後才改成本最高。
>
> **快速通過**：若 issue 已完整指定（spec 明確、無未明決策點）且只有一種合理做法 — 4a 回報「無未明決策點」並列出你採用的假設；4b 只列單一方案並一句話說明為何沒有值得比較的替代。不得為湊數而編造方案。快速通過時，Step 5b 仍需明確批准。
>
...
派工批准依情境二選一，**同一個決策不重複確認兩次**：

- **使用者已在 Step 4b 明確選定方案** → 該選定即為派工批准，不再問 Y/n。回報「即將依方案 {X} 派工，如要調整請現在喊停」後直接進入 Step 6。
- **未經 4b 選定**（Tier 1、Step 4 快速通過、或使用者在 4a/4b 答「你決定」）→ 明確徵求批准：
...
此情境下**收到明確批准（Y）才進入 Step 6**，未獲批准前不得派工或動 code。
